...
- **待人工复核**：候选资料中未发现明确的中文社区线索（如中文博客、知乎文章、B站教程等）。建议后续关注PawWork、EverFern、HexClaw Desktop等项目的国内社区讨论或汉化情况。

### 值得后续复核

- **PawWork (星数48)**：作为Codex App和Claude Cowork的替代品，功能全面，值得深入测试其桌面Agent能力。
- **EverFern (星数15)**：本地优先、无云端依赖，符合隐私敏感场景需求，值得关注其实际使用体验。
- **HexClaw Desktop (星数13)**：企业级定位，支持多模型和MCP插件，适合专业用户。
- **AmrDab/clawdcursor (星数342)**：MCP驱动的GUI回退层，星数较高，可能成为桌面Agent的通用基础设施。
- **OpenAI收购Ona**：可能影响Codex及企业Agent生态，需跟踪后续产品整合。
- **DeskCraft、Workflow-GYM、WeaveBench等新基准**：这些论文聚焦长周期、专业工作流评估，可能成为桌面Agent能力评估的新标准。
