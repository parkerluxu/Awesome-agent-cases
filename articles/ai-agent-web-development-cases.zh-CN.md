...
- **Human-in-the-Loop Swarms** 论文中的 Web 应用组件（智能手机 Web 应用作为人机接口）与 Agent 主题关联较弱，建议复核其与 Web Agent 开发的相关性。
- **Helicase** 论文聚焦供应链知识图谱，虽涉及多 Agent 和 Web 搜索，但非直接面向 Web 应用开发，建议复核是否纳入。
- **电子工程专辑** 的智能座舱文章属于垂直行业应用，与通用 Web 开发 Agent 的关联度待确认。
- 多个 GitHub 项目（autodemo、rails-skills、aigc-commerce-video-generator 等）Star 数为 0，社区活跃度低，建议人工评估代码质量和实用性。
- **RooCodeInc/Roo-Code** 的链接为 Google News 聚合页，非直接 GitHub 仓库，需人工确认最新仓库地址。
