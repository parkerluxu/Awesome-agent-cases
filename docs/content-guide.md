...
- `cases/` is generated from published Agent Case Share records. Do not edit generated case Markdown directly; update the case on the website and run `node scripts/sync-cases.mjs`.
- Do not create top-level `papers/` or `projects/` folders. Use `indexes/by-content-type.md` to provide those cross-cutting views.
...
- `cases/` 由 Agent Case Share 已发布案例自动生成。不要直接修改生成的 Markdown；请先在网站更新案例，再运行 `node scripts/sync-cases.mjs`。
- 不再新建顶层 `papers/`、`projects/`；跨类型入口统一维护在 `indexes/by-content-type.md`。
